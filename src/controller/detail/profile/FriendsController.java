...
public class FriendsController extends FxmlController {
    ...
    public void initForShow() {
    	...
    	cb_pseudonym.getEditor().setOnKeyReleased(event -> {
    		if(event.getCode() != KeyCode.UP & event.getCode() != KeyCode.DOWN & event.getCode() != KeyCode.LEFT & event.getCode() != KeyCode.RIGHT
    				& event.getCode() != KeyCode.KP_UP & event.getCode() != KeyCode.KP_DOWN & event.getCode() != KeyCode.KP_LEFT & event.getCode() != KeyCode.KP_RIGHT){
    			blendOut();
        		autoFill();
    		} else {
	...
	private void add(){
    	...
    	try {
    		newFriend = Session.getInstance().getPlayer().getPlayerByPseudonym(pseudonym);
		} catch (SQLException e) {
			e.printStackTrace();
		}
    	
    	if(newFriend == null){
    		lbl_error.setText("A problem occured, players couldn't be loaded");
	...
	private void remove(){
    	...
    	try {
			...
			lbl_error.setText("There couldn't be esteblashied a database conecction");
			lbl_error.setVisible(true);
			e.printStackTrace();
		}
    ...
    private ContextMenu prepareContextMenu(){
        ...
        addToGroup.setOnAction(event -> {
        	...
        	try {
				...
				Group friendsGroup = new Group(friend.getGroupId());
				if(myGroup.getLeaderIds().contains(mySelf.getId())){
					if(Player.getAllLoggedInPlayers().contains(friend)){
						friendsGroup.removePlayer(friend);
						myGroup.addPlayer(friend, false);
					} else {
				...
				lbl_error.setText("There was an Problem with database, player can not be added to group");
				lbl_error.setVisible(true);
				e.printStackTrace();
			}
        });
        ...
        addToTeam.setOnAction(event -> {
        	...
        	try {
				...
				lbl_error.setText("There was an Problem with database, teams can not be added to group");
				lbl_error.setVisible(true);
				e.printStackTrace();
			}
        	...
        	if(!teams.isEmpty()){
        		...
        		lbl_error.setText("You are in no Team or Teams couldn't be loaded");
        		lbl_error.setVisible(true);
        	}
